...
| KL9 | `README.md` and the `docs/manual/` pages it links out to (`jira-templates-and-cleanup-rules.md`, `settings-reference.md`, `bitbucket-pr-review.md`, `report-imports.md`) can drift out of sync — see `docs/plans/2026-09-21-1913-docs-readme-manual-split-plan.md` | 2026-09-22 | Low | Nothing enforces README-to-manual consistency automatically; the split relies on whoever edits either side to update the other. No functional or security impact, so it's tracked here for the monthly Routine to catch rather than built as an automated check. |
