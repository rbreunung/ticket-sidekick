...
| `src/participant/jira/reportImportHandler.ts` | Shared session-flow orchestration for all three report importers: template/issue-type selection, review-screen streaming, toggle-reply handling, batch ticket creation — parameterized per importer by a `ReportImportDescriptor` (parser, config reader, `buildTicketFields`, review-table columns, optional dedup fields, optional `afterCreate` post-creation hook, optional issue-type-fetch-failure UI callback). Dedup is optional — an importer with no dedup key (email) skips the "already ticketed" search entirely rather than running it and finding nothing. Owns the shared message wording (missing-template warning, cap/resume message, toggle-reply footer) so importers can't drift apart the way Veracode/Waltz did pre-consolidation |
