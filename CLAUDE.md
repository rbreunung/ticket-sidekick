...
| `src/bitbucket/BitbucketApiClient.ts` | Real HTTP; Data Center (Bearer PAT) + Cloud (Basic base64(username:apppassword)). `getPullRequestDiffWithCoverage`/`getPullRequestFileDiff` find and re-fetch files a truncated Data Center diff cut, and `summarizeDcDiffShape` logs each Data Center response's content-free shape (see `docs/review-process.md` "Data Center truncated diffs") |
...
| `src/participant/jira/uploadHandler.ts` | `@jira upload` chat flow: resolves the file from an explicit path, a chat-attached reference, or the active editor (falling back to a multi-select file picker) and the ticket from text, filename, or last-ticket context (falling back to an explicit ask), then confirms before uploading via `TicketService.uploadAttachment`. See `docs/jira-flows.md`'s "Attachment upload" section |
| `src/participant/jira/emailHandler.ts` | Batch email-to-ticket chat flow: the `email` `ReportImportDescriptor` (multi-select `.eml`, one shared template for the batch, per-email skip-and-continue creation, attachment upload via `afterCreate`) plus the untouched single-file "add email as a comment" flow (`EmailContentSession`) |
| `src/utils/emlParser.ts` | Parses `.eml` files via `postal-mime`; returns `ParsedEml` (subject, sender, date, htmlBody, plainBody, inlineImageMap, attachments). `parseEmlFile(filePath)` wraps one file into an `EmailImportItem` (markdown-converted body, flattened inline-image map) — the shared per-file parser all three `.eml` entry points use |
| `src/utils/reportImport.ts` | Shared, `vscode`-free primitives used by the Veracode, Waltz, and email importers: `chunkStrings`, `buildDedupJql` (always quotes labels), `extractDedupMap` (nested `{key, fields:{labels?}}[]` shape), `findAlreadyTicketed` (fault-tolerant per-chunk dedup search — one failed chunk doesn't discard matches from others), `capNewRows` (pre-cap-before-build, tracks `totalNewMatched`/`droppedOverCap`), `buildReviewRows`, `sanitizeCellText`/`sanitizeStandaloneLine` (untrusted-content sanitizers — the latter prefixes a value with `: ` before it becomes a standalone line, closing line-start injection vectors span-text stripping alone can't), the shared `MAX_REPORT_BYTES`/`BATCH_LIMIT` constants, and `resolveMaxReportBytes()` (turns a configured `ticketSidekick.<x>.maxReportSizeMB`/`maxBatchSizeMB` setting into a validated byte limit, falling back to a default when missing or out of range — the single source of truth every config-reading call site in `veracodeHandler.ts`/`waltzHandler.ts`/`emailHandler.ts`/`extension.ts` uses) |
| `src/participant/jira/reportImportHandler.ts` | Shared session-flow orchestration for all three report importers: template/issue-type selection, the overview-plus-per-group review screens (per-screen reply dispatch, one action per group: create / update / re-create / close, stale resolution asked only on close), batch ticket creation — parameterized per importer by a `ReportImportDescriptor` (parser, config reader, `buildTicketFields`, review-table columns, optional dedup fields, optional `afterCreate` post-creation hook, optional issue-type-fetch-failure UI callback). Dedup is optional — an importer with no dedup key (email) skips the "already ticketed" search entirely rather than running it and finding nothing. Owns the shared message wording (missing-template warning, cap/resume message, per-screen reply hints) so importers can't drift apart the way Veracode/Waltz did pre-consolidation |
...
| `src/participant/bitbucket/reviewDiagnostics.ts` | Diagnostics orchestration glue for the review pipeline's LLM call sites: `CallAttemptOut`/`OnCallAttemptError`/`CallDiagHooks` types, `errorCodeOf`, `handleAttemptFailure` (shared pass1/critic `onAttemptFailed` handling — logs the failure plus the R1/R2 per-call error line, then R5's recovery-decision line, gated on item-array identity against the original unsplit batch so a split half's terminal attempt never falsely logs a retry that won't happen); re-exports `createAttemptTracker`. No `vscode` import — pure and Vitest-loadable; `BitbucketParticipant.ts` injects its own logging as callbacks |
| `src/participant/sessionState.ts` | VS Code-free pure helpers and Jira multi-turn session types; includes `pickEmailOption()` for unified template+issue-type selection, `buildChatCommandLink()` (clickable-reply markdown links, never touches `vscode.MarkdownString`) and `neutralizeMarkdownLinks()` (its untrusted-content counterpart — see "Multi-turn session state") |
| `src/participant/reviewSessionState.ts` | Bitbucket session types + all pure review helpers: `parsePrUrl`, `hasPrUrl`, `parseDiff`, `parseReviewReply` (tolerant reply parser — NDJSON, pretty-printed, arrays, wrappers, cut-off replies), `parseFollowUpIntent` (parses multi-turn follow-up messages into a typed `FollowUpIntent` — `add` with `targets: number[] \| 'all'` and extracted note, or `explain` with cleaned `question` text and optional `findingRef`), `buildAdaptiveChunks`, `numberDiffLines` (render-only gutter), `locateAnchor` + `resolveFindingAnchors` (tiered match of the quoted `anchorCode` → verified line + provenance, or *location unverified*; drops and counts findings naming files outside the PR), `mergePass2Findings`, `buildStoredReviewDiff`, `buildFindingFollowUpPrompt`, `dedupeFindings`, `extractHunkAround`, `selectFilesWithinBudget`, `parseCriticKeep`, `createAttemptTracker`, `buildRunTag`, `formatCallLine`, `buildTruncationEvent`, `formatRecoveryDecision`, `formatFindingsFunnel`, `formatStructuredRunRecord`, `formatContinuationMessage` — the Bitbucket review pipeline's diagnostic-timeline formatters (see `docs/review-process.md` "Resilience & debugging") — plus its own `buildChatCommandLink()`/`neutralizeMarkdownLinks()` (duplicated from `sessionState.ts` rather than shared, since the two participants stay independent) |
| `src/utils/chatMarkdown.ts` | `trustedChatMarkdown(text)` — wraps a string in a `vscode.MarkdownString` trusted only for `workbench.action.chat.open`, the command every `buildChatCommandLink()` link points at. Any response containing one or more of those links must pass through this before `stream.markdown(...)` |
...
| `src/utils/lmRetry.ts` | 3-try retry for VS Code Language Model API calls: `withLmRetry` (identical-retry, for a single non-splittable prompt) and `withEasierRetry` (the 3rd try splits a batch in half instead of repeating it). `isTransientLmError` classifies which failures are worth retrying. `PartialLmResponseError` preserves any text a broken response stream had already sent; `UnparseableReplyError` marks a review reply with no usable JSON so it is retried like a provider error |
...
`JiraParticipant.ts` and `BitbucketParticipant.ts` import `vscode` and cannot be loaded by Vitest. Keep pure logic (string extraction, data formatting) in `sessionState.ts`, `reviewSessionState.ts`, `bitbucket/reviewDiagnostics.ts`, `TicketService.ts`, or `PrReviewService.ts` so it can be unit-tested. VS Code-dependent glue code is covered by the e2e suite only.
...
Multi-turn flows store structured state in `vscode.ExtensionContext.workspaceState` and carry an expiry signal through `ChatResult.metadata` (the same channel `JiraFollowupState`/`BitbucketFollowupState` already use for follow-up chips) rather than a visible marker in the response. Each session-producing response returns `{ metadata: { jiraSession: { kinds: [...] } } }` (Jira) or `{ metadata: { bitbucketSession: { kinds: [...] } } }` (Bitbucket); on the next turn, `getActiveJiraSession()`/`getActiveBitbucketSession()` reads that off the **last** turn in `chatContext.history`. If the user moved on (a different, non-session response is last), no metadata is present and the session is silently ignored — no HTML comment tag rides along in any rendered response.

Jira's session types plus their `workspaceState` keys and metadata kinds live in [`docs/jira-flows.md`](docs/jira-flows.md#jira-sessions) (also documents the clickable-reply mechanism every discrete-choice session uses). Bitbucket's `ReviewSession`, `BitbucketCommentPreviewSession`, and `SmartFallbackSession` — keys, kinds, detection order, and the PR-URL-bypass rule — live in [`docs/review-process.md`](docs/review-process.md#follow-ups).
...
`@jira` turns one or more `.eml` emails, a Veracode Detailed Report, or a
Waltz OSS Report into Jira tickets — all three sharing the same
template-pick/review/batch-creation flow via `reportImportHandler.ts`'s
`ReportImportDescriptor` (email as a dedup-less third kind). Documented in
